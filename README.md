...
**单片检视(Wafer Detail)**:页面底部新增面板,左侧列出全部 wafer(带良率色点,可搜索),
...
上传 STDF 后可查看每个测试项的**实测数据分布与统计**。

- **完整覆盖**:解析 **PTR**(单值参数)、**MPR**(多结果/多脚参数)、**FTR**(功能)三类测试记录。
  测试项以**名称**为身份识别(部分机台 TEST_NUM 恒为 0、靠 TEST_TXT 区分),不会漏项或误合并。
- **参数测试**:N、Mean、Std、Min/Max、Median、LO/HI、**Cp/Cpk**、超限点数/占比 +
  带 LO/HI/μ 参考线的**分布直方图**。直方图按 IQR 栅栏自动裁剪离群点(统计值仍基于全部点),
  使分布充满坐标轴、便于分析。
- **功能测试**:显示通过/失败数与失败率。
- 可按"全部 wafer"或单片过滤,并列出**片间均值/失败率对比**。
- 全程浏览器内解析(约 185MB / 190 万条测试记录,~1.3s)。统计工具见 `src/utils/paramStats.ts`。
